...
class ControllerPaymentOplata extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            $fondyParams = [
                ...
                'response_url' => $this->url->link('payment/oplata/response', '', true),
                'server_callback_url' => $this->url->link('payment/oplata/callback', '', true),
                'lang' => $lang,
                'sender_email' => $orderInfo['email'],
                'preauth' => $this->config->get('oplata_type') == 'preauth' ? 'Y' : 'N',
                'reservation_data' => $this->model_payment_oplata->getReservationData($orderInfo),
            ];

            $this->model_payment_oplata->addFondyOrder($fondyParams);

            if ($this->config->get('oplata_process_payment_type') == 'built_in_checkout') {
                $checkoutToken = $this->model_payment_oplata->getCheckoutToken($fondyParams);
                ...
                $data['checkout_url'] = $this->model_payment_oplata->getCheckoutUrl($fondyParams);
            }
        } catch (Exception $e) {
        ...
        return $this->load->view($this->getTemplateRoute('payment/oplata'), $data);
    }
    ...
    public function callback()
    {
        ...
        $fondyOrderInfo = $this->model_payment_oplata->getFondyOrder($fondyOrderID);
        ...
        $this->model_payment_oplata->updateFondyOrder($fondyOrderID, $this->request->post);
    }
    ...
    public function validate($request)
    {
        ...
        if ($this->config->get('oplata_environment') == 'test') {
            $merchantID = $this->model_payment_oplata->getTestMerchantID();
            $secretKey = $this->model_payment_oplata->getTestMerchantSecretKey();
        }
        ...
        $orderSignature = $this->model_payment_oplata->getSignature($request, $secretKey);
    ...
    private function getTemplateRoute($route)
    {
        if (version_compare(VERSION, '2.2.0.0', '<')){
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/oplata.tpl')) {
                $route = $this->config->get('config_template') . '/template/' . $route . '.tpl';
            } else {
                $route = 'default/template/' . $route . '.tpl';
            }
        }
