...
class ControllerExtensionPaymentFondyBank extends Controller
{
    ...
    public function index()
    {
        ...
        $backref = $this->url->link('extension/payment/fondy_bank/response', '', 'SSL');
        $callback = $this->url->link('extension/payment/fondy_bank/callback', '', 'SSL');
        $desc = $this->language->get('order_desq') . $order_id;
        if (($this->config->get('fondy_bank_currency'))) {
            $fondy_bank_currency = $this->config->get('fondy_bank_currency');
        } else {
            $fondy_bank_currency = $this->session->data['currency'];
        }
        ...
        $data['styles'] = $this->config->get('fondy_bank_styles');
        $data['button_confirm'] = $this->language->get('button_confirm');
        $this->load->model('checkout/order');
        if (version_compare(VERSION, '2.1.0.2', '>')) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/fondy_bank.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/extension/payment/fondy_bank.tpl', $data);
            } else {
                return $this->load->view('/extension/payment/fondy_bank.tpl', $data);
            }
        } else {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/fondy_bank.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/extension/payment/fondy_bank.tpl', $data);
            } else {
                return $this->load->view('default/template/extension/payment/fondy_bank.tpl', $data);
            }
        }
    ...
    public function callback()
    {
        ...
        if ($paymentInfo === true) {
            ...
            if ($this->request->post['order_status'] == $this->ORDER_APPROVED and $total == $this->request->post['amount']) {
                ...
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('fondy_bank_order_status_id'), $comment, $notify = true, $override = false);
                die('Ok');
                ...
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('fondy_bank_order_process_status_id'), $comment, $notify = false, $override = false);
                die($paymentInfo);
				...
				$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('fondy_bank_order_cancelled_status_id'), $comment, $notify = false, $override = false);
			}
        }
    }
