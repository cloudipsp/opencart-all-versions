...
class ControllerPaymentFondyBank extends Controller
{
    ...
    public function index()
    {
        ...
        $fondy_bank_args = array(
            ...
            'merchant_id' => trim($this->config->get('fondy_bank_merchant')),
            'order_desc' => $desc,
        ...
        if (version_compare(VERSION, '2.1.0.2', '>')) {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/fondy_bank.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/payment/fondy_bank.tpl', $data);
            } else {
                return $this->load->view('/payment/fondy_bank.tpl', $data);
            }
        } else {
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/fondy_bank.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/payment/fondy_bank.tpl', $data);
            } else {
                return $this->load->view('default/template/payment/fondy_bank.tpl', $data);
            }
        }
    }
	...
	public function generateFondyBankUrl($fondy_bank_args) {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(array('request' => $fondy_bank_args)));
